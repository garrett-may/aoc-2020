...
/*
 * This map is tailored to the input, as we happen to
 * know there are 71385 unique elements to be inserted
 *
 * The alternative to this would be a large array, which
 * is somewhat unfeasible
 */
struct TailoredMap{
    constexpr auto& operator[](std::size_t key) noexcept{
        std::size_t i = 0;
        for(auto& [k, v] : map[key % 4759]){
            if(k == key){
                return v;
            } else if(k == 0 && v == 0){
                map[key % 4759][i][0] = key;
                return map[key % 4759][i][1];
            }
            ++i;
        }
        return end;
    }
    constexpr auto const& operator[](std::size_t key) const noexcept{
        for(auto const& [k, v] : map[key % 4759]){
            if(k == key){
                return v;
            }
        }
    ...
    std::size_t end = 0;
    std::array<std::array<std::array<std::size_t, 2>, 30>, 4759> map{};
};
...
/*
 * From this we can tell how many 0s, 1s, and Xs were in the mask, and where
 */
constexpr auto read_mask_info(char const* line) noexcept{
    auto* str = line;
    std::size_t zeros_mask = 0;
    std::size_t ones_mask = 0;
    for(auto* str = line + 7; *str != '\0'; ++str){
        zeros_mask = (zeros_mask << 1) | std::size_t{*str == '0'};
        ones_mask = (ones_mask << 1) | std::size_t{*str == '1'};
    }
    return std::experimental::make_array(zeros_mask, ones_mask);
}

constexpr auto read_mem_info(char const* line) noexcept{
    auto* str = line;
    str += 4;
    auto index = str_to_size_t(str, &str);
    str += 4;
    auto value = str_to_size_t(str, &str);
    return std::experimental::make_array(index, value);
}

// part 1

constexpr void modify(TailoredMap& memory, std::array<std::size_t, 2> const& mem_info, std::array<std::size_t, 2> const& mask_info) noexcept{
    auto [index, value] = mem_info;
    auto const& [zeros_mask, ones_mask] = mask_info;
    value = (value | ones_mask) & ~zeros_mask;
    memory[index] = value;
}

// part 2

constexpr void decode_rec(TailoredMap& memory, std::size_t index, std::size_t value, std::size_t xs_mask, std::size_t i = 0) noexcept{
    if(i < 36){
        auto bit = std::size_t{1} << i;
        auto mask = xs_mask & bit;
        if(mask != 0){
            auto a = index & ~mask;
            auto b = index | mask;
            decode_rec(memory, a, value, xs_mask, i + 1);
            decode_rec(memory, b, value, xs_mask, i + 1);
        } else{
            decode_rec(memory, index, value, xs_mask, i + 1);
        }
    } else{
        memory[index] = value;
    }
...
constexpr void decode(TailoredMap& memory, std::array<std::size_t, 2> const& mem_info, std::array<std::size_t, 2> const& mask_info) noexcept{
    auto [index, value] = mem_info;
    auto const& [zeros_mask, ones_mask] = mask_info;
    // Replace leading 1s with leading 0s due to `sizeof(std::size_t) > 36`
    auto leading_zeros = ~(std::numeric_limits<std::size_t>::max() << 36);
    auto xs_mask = leading_zeros & ~(zeros_mask | ones_mask);
    index |= ones_mask;
    decode_rec(memory, index, value, xs_mask);
}

// Altogether

template<auto f, typename Array>
constexpr auto memory_sum(Array const& array) noexcept{
    auto mask_info = std::array<std::size_t, 2>{};
    auto map = TailoredMap{};
    for(auto const* line : array){
        if(std::string_view{line, 3} == std::string_view{"mas"}){
            // mask
            mask_info = read_mask_info(line);
        } else{
            // mem
            auto mem_info = read_mem_info(line);
            f(map, mem_info, mask_info);
        }
    }
    std::size_t sum = 0;
    for(auto const& row : map.map){
...
constexpr static auto sum_modify = memory_sum<modify>(input);
constexpr static auto sum_decode = memory_sum<decode>(input);

int main(){
    std::printf("sum modify = %lu\n", sum_modify);
    std::printf("sum decode = %lu\n", sum_decode);
    return 0;
}
