...
// Defining separate Bags, Parent, and Child structs to
// avoid any confusion involving the graphs later on
...
/*
 * Register - an array of indices mapping to colours
 *            an "alternative" to a compile-time map,
 *            assuming this will be fast enough
 */
template<std::size_t size>
constexpr auto make_register(std::array<char const*, size> const& array) noexcept{
    return std::apply([](auto const*... strs){
        return std::experimental::make_array([](auto const* str){
            auto view = std::get<0>(split(str, " contain "));
            return std::string_view{view.data(), view.size() - 5};
        }(strs)...);
    }, array);
}

/*
 * Find the index of a colour in the register given the colour name
 * Assuming this will be fast enough
 */
template<typename Array>
constexpr std::size_t find_in_register(Array const& r, std::string_view view) noexcept{
    std::size_t i = 0;
    for(auto item : r){
        if(item == view){
            return i;
...
/*
 * Parsing '(number|"no other")" bag(s?)"'
 */
template<typename Array>
constexpr auto parse_bags(Array const& r, std::string_view view) noexcept{
    auto [amount, str] = split(view, " ");
    if(amount != std::string_view{"no"}){
        auto number = std::size_t(str_to_int(amount.data(), nullptr));
        auto colour = find_in_register(r, std::get<0>(split(str, " bag")));
        return Bags{number, colour};
    } else{
        return Bags{};
    }
}
...
/*
 * Given the input, produce two graphs:
 * - graph mapping parent colours -> child colours (each bag -> bags inside this bag)
 * - graph mapping child colours -> parent colours (each bag -> bags that have this bag inside)
 */
template<std::size_t size>
constexpr auto make_graphs(std::array<char const*, size> const& array) noexcept{
    auto r = make_register(array);
    auto c_to_p_graph = std::array<std::array<Parent, 32>, size>{}; // "compile-time list" of size 32 is good enough
    auto p_to_c_graph = std::array<std::array<Child, 4>, size>{};   // "compile-time list" of size 4 is good enough
    std::size_t i = 0;
    for(auto const* line : array){
        auto str = std::get<1>(split(line, " contain "));
        std::size_t k = 0;
        while(str.size() > 0){
            auto [part, rest] = split(str, ", ");
            auto bags = parse_bags(r, part);
            if(bags.number > 0){
                std::size_t j = 0;
                while(c_to_p_graph[bags.colour][j].number > 0){
                    ++j;
                }
                c_to_p_graph[bags.colour][j] = Parent{bags.number, i};
                p_to_c_graph[i][k] = Child{bags.number, bags.colour};
                ++k;
            }
            str = rest;
        }
    ...
    return std::make_tuple(r, c_to_p_graph, p_to_c_graph);
}

template<typename Graph, typename Set>
constexpr void search_parents(Graph const& graph, Set& set, std::size_t colour) noexcept{
    for(auto item : graph[colour]){
        if(item.number > 0){
            if(!set[item.parent]){
                set[item.parent] = true;
                search_parents(graph, set, item.parent);
            }
        }
    }
}
...
constexpr std::size_t search_children(Graph const& graph, std::size_t colour) noexcept{
    std::size_t n = 1;
    for(auto item : graph[colour]){
        if(item.number > 0){
            n += item.number * search_children(graph, item.child);;
        }
    }
    return n;
}
...
template<std::size_t size>
constexpr auto search(std::array<char const*, size> const& array, std::string_view view) noexcept{
    auto [r, c_to_p_graph, p_to_c_graph] = make_graphs(array);
    auto colour = find_in_register(r, view);

    // Part 1
    // using a "compile-time set" to avoid duplicates during recursion
    auto set = std::array<bool, size>{};
    search_parents(c_to_p_graph, set, colour);
    std::size_t n = 0;
    for(auto x : set){
        n += std::size_t(x);
    }
    ...
    // Part 2
    // Take 1 away to not include the original shiny gold bag
    auto m = search_children(p_to_c_graph, colour) - 1;
...
constexpr static auto answer = search(input, "shiny gold");

int main(){
    std::printf("bag colours that can contain one shiny gold bag = %lu\n", std::get<0>(answer));
    std::printf("bag colours required inside one shiny gold bag  = %lu\n", std::get<1>(answer));
    return 0;
}
